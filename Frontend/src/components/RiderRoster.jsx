    ...
    if (riders.length === 0) {
        return (
            <section className="bg-slate-panel rounded-xl overflow-hidden border border-slate-700">
                ...
                <div className="p-6 text-center space-y-2">
                    <span className="material-symbols-outlined text-slate-500 text-3xl">sports_motorsports</span>
                    <p className="text-slate-500 text-sm">No riders currently available.</p>
                </div>
                <div className="px-6 py-4 border-t border-slate-700 flex justify-center bg-slate-800/30 w-full">
                    <Link to="/riders" className="text-sm font-medium text-emerald-500 hover:text-emerald-400 uppercase tracking-wide flex items-center gap-1 transition-colors">
                        View All Riders
                ...
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-700 text-sm">
                        {displayRiders.map(rider => (
                            <tr key={rider.phone} className="hover:bg-slate-800/40 transition-colors">
                                <td className="px-6 py-4 font-medium">{rider.name}</td>
                                <td className="px-6 py-4 text-slate-400">{rider.phone}</td>
                                <td className="px-6 py-4">
                                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${rider.status === 'AVAILABLE' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20'}`}>
                                        {rider.status}
                                    </span>
                                </td>
                                <td className="px-6 py-4">{rider.current_location} ({rider.location_code})</td>
                                <td className="px-6 py-4">
                                    {rider.is_verified && <span className="material-symbols-outlined text-emerald-500 text-lg">verified</span>}
                                </td>
                            </tr>
                        ))}
                    </tbody>
